...
public class BidServiceTests
{
    ...
    {
        var dto = new BidDto { Id = 1 };
        var existing = new Bid { Id = 1 };
        var updated = new Bid { Id = 1 };
        var resultDto = new BidDto { Id = 1 };

        _repoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(existing);
        _mapperMock.Setup(m => m.Map<Bid>(dto)).Returns(updated);
        _mapperMock.Setup(m => m.Map<BidDto>(updated)).Returns(resultDto);
    ...
    [Fact]
    public async Task DeleteAsync_Should_Return_False_When_Bid_Not_Found()
    {
        _repoMock.Setup(r => r.GetByIdAsync(It.IsAny<int>())).ReturnsAsync((Bid)null);
